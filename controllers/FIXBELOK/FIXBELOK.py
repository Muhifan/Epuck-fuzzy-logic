...
while robot.step(TIME_STEP) != -1:
    ...
    if jarak_tengah_value <= 750:  # Jika ada objek di depan (PS10 membaca 502)
        ...
        left_motor.setVelocity(-MAX_SPEED * 1)
        right_motor.setVelocity(MAX_SPEED * 1)
    ...
    # Simpan data kecepatan untuk plot
        motor_kiri_data.append(left_motor)
        motor_kanan_data.append(right_motor)

# Plot grafik kecepatan motor setelah loop utama selesai
plt.figure()
plt.plot(motor_kiri_data, label='Motor Kiri')
plt.plot(motor_kanan_data, label='Motor Kanan')
plt.xlabel('Waktu')
plt.ylabel('Kecepatan Motor')
plt.title('Kecepatan Motor Kiri dan Kanan')
plt.legend()
plt.show()
plt.pause(1)