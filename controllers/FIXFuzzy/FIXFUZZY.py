...
while robot.step(TIME_STEP) != -1:
    ...
    # Sensor depan kiri
    sensor_depan_kiri_dekat = fuzz.trapmf(x_sensor_depan_kiri, [0, 0, 650, 700])
    ...
    # Sensor depan tengah
    sensor_depan_tengah_dekat = fuzz.trapmf(x_sensor_depan_tengah, [0, 0, 650, 700])
    ...
    #Kecepatan kanan 
    lambat_kanan = fuzz.trapmf(x_kategori_kanan, [0, 0, 1, 2])
    normal_kanan = fuzz.trapmf(x_kategori_kanan, [1, 2, 4, 5])
    cepat_kanan = fuzz.trapmf(x_kategori_kanan, [4, 5, 6.28, 6.28])

    #kecepatan Kiri
    lambat_kiri = fuzz.trapmf(x_kategori_kiri, [0, 0, 1, 2])
    normal_kiri = fuzz.trapmf(x_kategori_kiri, [1, 2, 4, 5])
    ...
    Persentase_lambat_kanan = np.fmin(np.fmax(rules_1, np.fmax(rules_2, np.fmax(rules_3, np.fmax(rules_4, np.fmax(rules_7, np.fmax(rules_10, np.fmax(rules_11, np.fmax(rules_12, np.fmax(rules_20, rules_21))))))))), lambat_kanan)
    ...
    Persentase_cepat_kanan = np.fmin(np.fmax(rules_13, np.fmax(rules_16, np.fmax(rules_19, np.fmax(rules_22, np.fmax(rules_25, rules_27))))), cepat_kanan)

    # Pemggabungan motor kiri
    Persentase_lambat_kiri = np.fmin(np.fmax(rules_1, np.fmax(rules_4, np.fmax(rules_7, np.fmax(rules_10, np.fmax(rules_11, np.fmax(rules_12, np.fmax(rules_16, np.fmax(rules_19, np.fmax(rules_20, rules_21))))))))), lambat_kiri)
    ...
    Persentase_cepat_kiri = np.fmin(np.fmax(rules_2, np.fmax(rules_3, np.fmax(rules_5, np.fmax(rules_6, np.fmax(rules_8, np.fmax(rules_9, rules_27)))))), cepat_kiri)

    hasil_kanan = np.zeros_like(x_kategori_kanan)
    hasil_kiri = np.zeros_like(x_kategori_kiri)

    # Defuzzifikasi kanan
    aggregated_kanan = np.fmax(Persentase_lambat_kanan, np.fmax(Persentase_normal_kanan, Persentase_cepat_kanan))
    ...
    # Defuzzifikasi kiri
    aggregated_kiri = np.fmax(Persentase_lambat_kiri, np.fmax(Persentase_normal_kiri, Persentase_cepat_kiri))
    ...
    if jarak_tengah_value == 0 and jarak_kanan_value == 0 and jarak_kiri_value == 0:
            motor_kiri = normal_speed
            motor_kanan = normal_speed
