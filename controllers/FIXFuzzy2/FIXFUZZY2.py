...
while robot.step(TIME_STEP) != -1:
    ...
    # Range motor
    x_kategori_kanan = np.arange(-1, 6.28, 0.1)
    x_kategori_kiri = np.arange(-1, 6.28, 0.1)
    ...
    #Kecepatan kanan 
    stop_kanan = fuzz.trimf(x_kategori_kanan, [-1, 0, 1])
    ...
    #kecepatan Kiri
    stop_kiri = fuzz.trimf(x_kategori_kiri, [-1, 0, 1])
    ...
    Persentase_stop_kanan = np.fmin(rules_1, stop_kanan)
    Persentase_lambat_kanan = np.fmin(np.fmax(rules_2, np.fmax(rules_3, np.fmax(rules_4, np.fmax(rules_7, np.fmax(rules_10, np.fmax(rules_11, np.fmax(rules_12, np.fmax(rules_20, rules_21)))))))), lambat_kanan)
    ...
    Persentase_stop_kiri = np.fmin(rules_1, stop_kiri)
    Persentase_lambat_kiri = np.fmin(np.fmax(rules_4, np.fmax(rules_7, np.fmax(rules_10, np.fmax(rules_11, np.fmax(rules_12, np.fmax(rules_16, np.fmax(rules_19, np.fmax(rules_20, rules_21)))))))), lambat_kiri)
